...
| [doc-drift-watch](plugins/doc-drift-watch) | After each commit the model makes, adds the doc lines that commit made stale (file:line references, symbol names) to the commit's result, from [ripwire](https://github.com/redhat-et/ripwire) doc-drift. | L2 |
...
| [mod-doctor](plugins/mod-doctor) | Names each installed mod of a marketplace whose local clone already offers a newer version, with the `claude plugin update` command that closes the gap. | L1 |
| [subagent-ledger](plugins/subagent-ledger) | Shows each subagent of the session with its turns, time, model and tokens, the costliest first, and marks one that passed the token limit. | L0 |
