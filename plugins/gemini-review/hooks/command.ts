/** The settings /gemini-review changes, and the reading of its argument. */
...
export const FREE_WARNING =
  'free tier: Google may use the conversation and the diff sent to Gemini to improve its products, and human reviewers may read them (Gemini API Additional Terms). Use paid with a billing-enabled key to avoid this.'

/** The line /gemini-review prints for a change. */
export function changeText(patch: Patch): string {
  if (patch.enabled !== undefined) return patch.enabled ? 'on: every commit the model makes is reviewed' : 'off: commits run without a review'
  if (patch.tier !== undefined) return patch.tier === 'free' ? FREE_WARNING : 'paid tier'
  return `model ${patch.model ?? ''}`
}

/** The status of /gemini-review. */
