...
Reach L2, drives Claude.
    ...
    4. Persists: in $.store, the window end, the ping period and the last main-loop request's time under this session's id, and the global always switch, which the endless loop needs no window key beside; this session's ended window is deleted at stop and at its next start, another session's window one week after it ended, another session's request time once it is an hour old; the cold-write tally lives in memory and ends with the session
