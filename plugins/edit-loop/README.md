...
Validated with `claude plugin validate` on Claude Code 2.1.278:
    ...
    ❯ ./register.ts calls: $.command.register, $.session.cwd (via afterEdit), $.store.get, $.store.set (via runCommand)

Reach L0, remembers.

    1. Reads:    the file path of each Edit, Write and NotebookEdit call; the session's directory
    2. Runs:     nothing
    3. Sends:    a note to the model after the fifth edit of one file in a turn; nothing leaves the machine
