...
/** What a Gemini mod runs with; `thinking` absent is the model's own default. */
export type GeminiSettings = { hasKey: boolean; tier: GeminiTier; model: string; thinking?: GeminiThinking }

/** A generateContent POST with the key in a header, never in the URL. */
export type GeminiHttp = { url: string; init: { method: 'POST'; headers: Record<string, string>; body: string } }
...
/** One HTTP response and where the caller is in its attempts. */
...
/** `read`'s answer: the answer, why there is none, or how long to wait before sending the same request again. */
export type GeminiRead = { answer: GeminiAnswer } | { error: string } | { retryInMs: number }
...
export type Gemini = {
  ...
  /** Reads a response; asks for another attempt after an HTTP 503 while the deadline allows. */
