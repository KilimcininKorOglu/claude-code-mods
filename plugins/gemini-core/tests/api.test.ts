...
describe('the command', () => {
  test('reads the tier, a mod\'s model and thinking level, and refuses the rest', async () => {
    ...
    expect(parseCommand('model review')).toEqual({ kind: 'pick', consumer: 'review' })
    expect(parseCommand('models')).toEqual({ kind: 'models', refresh: false })
    expect(parseCommand('models refresh')).toEqual({ kind: 'models', refresh: true })
    for (const bad of ['model', 'models all', 'models refresh now']) expect(parseCommand(bad).kind, bad).toBe('error')
