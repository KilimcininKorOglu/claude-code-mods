...
import { denyText, doneLines, doneLog, isCommit, isGuarded, isNarrowable, isSource, logText, modeOf, noteText, openNote, placesOf, sectionKey, shownPath, sidebarLines, stillUsed, storageLines, type Line, type Mode } from './storage.ts'
...
/** One file's open finding: the file on disk, the lines that use the storage, and where they stand in it. */
...
/** The file's text, or undefined when it cannot be read; the first failure is logged. */
async function fileText($: EngineInterface, state: State, path: string): Promise<string | undefined> {
  try {
    return await $.fs.read(path)
  } catch (err) {
    if (!state.reported) $.ui.log(`the edited file was not read, its places stay as reported: ${err instanceof Error ? err.message : String(err)}`)
    state.reported = true
    return undefined
  }
}
 ...
 * transcript line. The model's note is another channel and does not change here.
 */
async function toPerson($: EngineInterface, key: string, title: string, lines: Line[], line: string): Promise<void> {
  try {
    const taken = await $.sidebar.set({ consumer: 'storage-guard', key: sectionKey(key), title, lines, until: 'stream' })
    if (taken) return
  } catch {
...
/** Drops the sidebar entries of one finding, so a file that no longer uses the storage leaves no warning behind. */
async function dropEntry($: EngineInterface, key: string): Promise<void> {
  try {
    await $.sidebar.clear({ consumer: 'storage-guard', key: sectionKey(key) })
  } catch {
    // The sidebar mod is not installed.
  }
}
...
async function closeResolved($: EngineInterface, state: State, skip?: string): Promise<void> {
  for (const [shown, found] of [...state.open]) {
    ...
    const left = stillUsed(text, found.lines)
    if (left.length > 0) {
    ...
    await toPerson($, shown, 'cookie used instead', doneLines(shown, found.places), doneLog(shown, found.places))
  }
}

/** The note of one edit and the file it named, and the finding it opens; undefined when the edit adds no storage use. */
async function noteFor($: EngineInterface, state: State, path: string, before: string, after: string): Promise<{ note: string; shown: string } | undefined> {
  const lines = storageLines(before, after)
  ...
  const kept = text === undefined ? held : stillUsed(text, held)
  ...
  await toPerson($, shown, 'browser storage instead of a cookie', sidebarLines(places), logText(places))
  return { note: noteText(places), shown }
}

/** Adds the note to an edit whose new lines use the storage, and closes what a later edit fixed. */
 ...
 * finding opens the gate too. A file this command holds that still uses the storage stops it, and there
 * is no bypass.
 */
async function gate($: EngineInterface, state: State, command: string): Promise<string | undefined> {
  ...
  if (scoped.length === 0) {
    $.ui.log(`${state.open.size} file(s) still use localStorage or sessionStorage, and this command holds none of them`)
    return undefined
  }
...
async function setMode($: EngineInterface, state: State, word: string): Promise<string> {
  ...
  state.mode = mode
  return mode === 'deny' ? 'mode deny: git commit, push and merge stop while a file uses localStorage or sessionStorage' : 'mode note: the places are only reported'
}

function statusText(state: State): string {
  const open = state.open.size === 0 ? 'no file is open' : `${state.open.size} file(s) still use localStorage or sessionStorage`
...
async function runCommand($: EngineInterface, state: State, args: string): Promise<string> {
  ...
  if (word === 'on' || word === 'off') {
    ...
    return word === 'on' ? 'on: each edit is checked for localStorage and sessionStorage' : 'off: edits are not checked'
  }
...
export const register: Register = on => {
  ...
  on('command.run', { command: 'storage-guard' }, async ($, e) => ({ text: await runCommand($, state, String(e.args ?? '')) }))
