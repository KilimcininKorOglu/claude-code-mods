...
describe('gemini-advisor', () => {
  ...
  it('sends the same request with the next key at once after a 429', async ($, on) => {
    ...
    w.replies.push({ status: 429, text: JSON.stringify({ error: { message: 'quota' } }) }, { status: 200, text: reply('Fine.') })
    expect((await $.tool.call({ tool: TOOL, message: 'x' })).result).toBe('Fine.')
