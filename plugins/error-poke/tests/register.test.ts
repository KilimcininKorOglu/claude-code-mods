...
import { decide, MAX_POKES, POKE_TEXT, statusText } from '../hooks/poke.ts'
...
describe('error-poke', () => {
  test('only an API error asks for a continue prompt', () => {
    expect(decide('error', 0)).toBe('poke')
    expect(decide('error', MAX_POKES)).toBe('limit')
    for (const reason of ['answer', 'aborted', 'refusal']) expect(decide(reason, 0), reason).toBe('idle')
    expect(statusText(true, 2, 'error')).toBe('on · 2/5 continue prompts since your last prompt · last turn: error')
    expect(statusText(false, 0, undefined)).toBe('off · 0/5 continue prompts since your last prompt · no turn has ended yet')
  })
  ...
  test('a turn killed by an API error gets one continue prompt, and the person reads one line', async ($, on) => {
    ...
    expect(w.logs).toEqual(['the turn died on an API error, continuing (1/5)'])
    expect((await $.command.run(run(''))).text).toBe('on · 1/5 continue prompts since your last prompt · last turn: error')
  })
  ...
  test('the prompts stop at the limit, say so once, and a prompt of the person resets the count', async ($, on) => {
    const w = world(on)
    await started($)
    for (let i = 0; i < MAX_POKES + 2; i++) await ended($, 'error', `t${i}`)
    expect(w.sent).toHaveLength(MAX_POKES)
    expect(w.logs.filter(l => l.startsWith('stopped after'))).toEqual([
      'stopped after 5 continue prompts; the API keeps failing. Send a prompt to reset the count.',
    ])
    await $.prompt.submit({ text: 'go on then', wait: false, origin: { kind: 'composer' } })
    expect((await $.command.run(run(''))).text).toContain('0/5 continue prompts')
    await ended($, 'error', 'again')
    expect(w.sent).toHaveLength(MAX_POKES + 2)
  })

  test('off sends nothing, and on sends again', async ($, on) => {
    const w = world(on)
    await started($)
    expect((await $.command.run(run('off'))).text).toContain('off · 0/5')
    await ended($, 'error')
    expect(w.sent).toEqual([])
  ...
  withSidebar('an open sidebar takes the lines and the transcript stays clean', async ($, on) => {
    ...
    await started($)
    await ended($, 'error')
    expect(bar.sections).toEqual([{ key: 'poke-1', title: 'turn continued after an API error', lines: ['the turn died on an API error, continuing (1/5)'] }])
    expect(w.logs).toEqual([])
  })
})
